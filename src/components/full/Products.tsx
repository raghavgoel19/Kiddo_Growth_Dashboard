...
function computeTopByVolume(boardOrders: Order[]) {
...
function computeCategoryRevenue(boardOrders: Order[], productTagsMap: ProductTagsMap) {
  const map = new Map<string, number>()
  for (const order of boardOrders) {
...
function computeL2Table(boardOrders: Order[], productTagsMap: ProductTagsMap) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <SectionCard title="Top 10 by revenue" orders={orders} enableBoardDateFilter defaultBoardPreset="30d">
          {(boardOrders) => {
            const topByRevenue = computeTopProducts(boardOrders).slice(0, 10)
            return (
        ...
        <SectionCard title="Top 10 by volume" orders={orders} enableBoardDateFilter defaultBoardPreset="30d">
          {(boardOrders) => (
            <ResponsiveContainer width="100%" height={320}>
              <BarChart data={computeTopByVolume(boardOrders).map((d) => ({ ...d, short: d.productTitle.slice(0, 28) }))} layout="vertical">
                <CartesianGrid horizontal={false} strokeDasharray="3 3" />
                <XAxis type="number" />
                <YAxis type="category" dataKey="short" width={120} tick={{ fontSize: 10 }} />
                <Tooltip />
      ...
      <SectionCard title="L1 category revenue" orders={orders} enableBoardDateFilter defaultBoardPreset="30d">
        {(boardOrders) => (
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={computeCategoryRevenue(boardOrders, productTagsMap)}>
      ...
      <SectionCard
        title="L2 sub-category breakdown"
        metricKey="l2Products"
        orders={orders}
        enableBoardDateFilter
        defaultBoardPreset="30d"
      >
        {(boardOrders) => {
          const l2Table = computeL2Table(boardOrders, productTagsMap)
          return (
            <>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[480px] text-sm">
                  ...
                  <TableSummaryFooter
                    cells={[
                      { type: 'text', values: [] },
                      { type: 'orders', values: l2Table.map((r) => r.orders) },
                      { type: 'currency', values: l2Table.map((r) => r.gmv) },
                      { type: 'aov', values: l2Table.map((r) => r.aov) },
                    ]}
                  />
                </table>
              </div>
