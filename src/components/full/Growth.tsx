...
function computeMonthlyGmv(boardOrders: Order[]) {
...
function computeCategoryMomentum(boardOrders: Order[], productTagsMap: ProductTagsMap) {
...
export function GrowthTab({ orders, customers, productTagsMap }: TabProps) {
  const wow = useMemo(() => {
    ...
    const thisWeek = orders.filter((o) => inRange(o, thisWeekStart, now))
    const prevWeek = orders.filter((o) => inRange(o, lastWeekStart, lastWeekEnd))
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <SectionCard title="Monthly GMV" orders={orders} enableBoardDateFilter defaultBoardPreset="30d">
          {(boardOrders) => (
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={computeMonthlyGmv(boardOrders)}>
      ...
      <SectionCard
        title="Category momentum (MoM order growth)"
        metricKey="momentum"
        orders={orders}
      ...
      >
        {(boardOrders) => {
          const categoryMomentum = computeCategoryMomentum(boardOrders, productTagsMap)
          return (
            <>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[400px] text-sm">
                  ...
                  <TableSummaryFooter
                    cells={[
                      { type: 'text', values: [] },
                      { type: 'orders', values: categoryMomentum.map((r) => r.orders) },
                      { type: 'percentage', values: categoryMomentum.map((r) => r.momGrowth) },
                    ]}
                  />
                </table>
              </div>
